...
module.exports = function (source, options) {
  ...
  if(source.postData.mimeType === 'application/json') {
    paramNames = paramNames.concat(swaggerMethod.parameters.map(function(param) {
      if(param.in === 'body') {
        code.push('var %s = new %s();', param.name, camelcase(param.name, { pascalCase: true }))
        return param.name
      }
    }))
  }
  ...
  if(filteredParamNames.length > 0) {
    code.blank()
        .push('var result = api.%s(%s);', operationId, filteredParamNames.join(', '))
  } else {
    code.blank()
        .push('var result = api.%s();', operationId)
  }
